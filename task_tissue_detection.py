...
from new_dataset_classification import Region_5Classes_Feat, get_train_test_ds_MultiCenter_region_5Cls
import datetime
import utliz
import random
...
def train_one_epoch(epoch, model, loader, optimizer, dev, writer):
    model = model.train()
    XE = torch.nn.CrossEntropyLoss(weight=torch.tensor([1.0, 1.0, 5.0, 5.0, 10.0]).to(dev))
    ...
    threshold4, best_tpr, best_fpr = find_best_threshold_youden(binary_y_true, conf_class4)
...
def export_slide_info_to_excel(internal_train, internal_test, external, huadong, output_path="slide_info.xlsx"):
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, args.epochs):
        threshold = train_one_epoch(epoch=epoch, model=model, loader=train_loader, optimizer=optimizer, dev=dev, writer=writer)
